...
# ─────────────────────────────────────────────
# File upload (must happen BEFORE data loading)
# ─────────────────────────────────────────────
with st.sidebar:
...
# ─────────────────────────────────────────────
# Auto-compute optimal bid from data
# ─────────────────────────────────────────────
@st.cache_data
def find_optimal_bid(mu_log, sigma_log, bid_prep_bytes,
                     c1_min, c1_mode, c1_max, c2_prob, c2_min, c2_mode, c2_max):
    """Quick sweep at 1k granularity to find the expected-profit-maximising bid."""
    bp = np.frombuffer(bid_prep_bytes, dtype=np.float64)
    bids = np.arange(90_000, 185_001, 1_000)
    best_bid, best_ep = bids[0], -np.inf
    N = 30_000
    np.random.seed(0)
    for b in bids:
        bp_s  = np.random.choice(bp, size=N, replace=True)
        pc_s  = np.maximum(70_000, np.exp(np.random.normal(mu_log, sigma_log, N)))
        c1    = np.random.triangular(c1_min, c1_mode, c1_max, N)
        e2    = np.random.random(N) < c2_prob
        e3    = np.random.random(N) < c2_prob
        c2    = np.where(e2, np.random.triangular(c2_min, c2_mode, c2_max, N), np.inf)
        c3    = np.where(e3, np.random.triangular(c2_min, c2_mode, c2_max, N), np.inf)
        wins  = b < np.minimum(np.minimum(c1, c2), c3)
        ep    = np.where(wins, b - pc_s - bp_s, -bp_s).mean()
        if ep > best_ep:
            best_ep, best_bid = ep, b
    return int(best_bid), float(best_ep)

_bp_bytes = bid_prep_data.astype(np.float64).tobytes()
auto_optimal_bid, auto_optimal_ep = find_optimal_bid(
    mu_log, sigma_log, _bp_bytes,
    90_000, 130_000, 180_000, 0.5, 90_000, 130_000, 180_000
)

# ─────────────────────────────────────────────
# Rest of sidebar (now auto_optimal_bid is defined)
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown("---")
    ...
    st.markdown(f'<div style="background:#e6f4ea;border-radius:8px;padding:0.6rem 0.9rem;font-size:0.85rem;color:#1e5631;margin-bottom:0.5rem;">🏆 <b>Recommended bid: ${auto_optimal_bid:,.0f}</b><br><span style="font-size:0.78rem;">Auto-calculated from your data. Adjust below to explore.</span></div>', unsafe_allow_html=True)
    miller_bid = st.number_input(
        "Miller's Bid Amount ($)", min_value=70_000, max_value=500_000,
        value=auto_optimal_bid, step=1_000,
        help="Pre-filled with the optimal bid. Adjust to explore other amounts."
    )
...
# ─────────────────────────────────────────────
# ─────────────────────────────────────────────
# Top recommendation banner (always visible)
# ─────────────────────────────────────────────
st.markdown(f'''
<div style="background:linear-gradient(135deg,#1e5631,#27ae60);color:white;border-radius:12px;
            padding:1.2rem 1.6rem;margin-bottom:1.2rem;display:flex;align-items:center;gap:1.5rem;">
  <div style="font-size:2.8rem;line-height:1;">🏆</div>
  <div>
    <div style="font-size:0.85rem;opacity:0.85;margin-bottom:0.2rem;">RECOMMENDED BID (auto-calculated from your data)</div>
    <div style="font-size:2.2rem;font-weight:800;letter-spacing:-1px;">${auto_optimal_bid:,.0f}</div>
    <div style="font-size:0.82rem;opacity:0.8;margin-top:0.2rem;">
      Maximises expected profit · Base scenario · Adjust competitor settings in sidebar to recalculate
    </div>
  </div>
</div>
''', unsafe_allow_html=True)
