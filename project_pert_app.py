...
def draw_network_diagram(activities, critical_path):
    ...
    non_crit_nodes = [n for n in G.nodes() if n not in crit_set]
    nx.draw_networkx_nodes(G, pos, nodelist=non_crit_nodes, ax=ax, node_color="#e8f5f7", node_size=1800, edgecolors=TEAL, linewidths=2)
    ...
    for node, (x, y) in pos.items():
        ...
        name = activities[node]["name"]
        short = name if len(name) <= 14 else name[:12] + "…"
        ax.text(x, y - 0.28, short, ha="center", va="center", fontsize=7.5, color="#4a6a72")
    crit_patch = mpatches.Patch(color=RED, label="Critical Path")
    norm_patch = mpatches.Patch(facecolor="#e8f5f7", edgecolor=TEAL, linewidth=2, label="Non-Critical")
    ax.legend(handles=[crit_patch, norm_patch], loc="upper left", framealpha=0.9, fontsize=9, facecolor="white", edgecolor="#e0eef1")
...
def export_word(activities, durations, pert_dur, crit_path, sl, unit):
    ...
    for i, h in enumerate(["Metric", "Value"]):
        ...
        shd.set(qn("w:val"),"clear"); shd.set(qn("w:color"),"auto"); shd.set(qn("w:fill"),"0a7e8c"); tcPr.append(shd)
    for row_idx, (label, value) in enumerate([("Mean Project Duration", f"{mean_dur:.2f} {unit}"), ("Standard Deviation", f"{std_dur:.2f} {unit}"), (f"{sl}% Service Level", f"{sl_val:.2f} {unit}"), ("PERT Expected Duration", f"{pert_dur:.2f} {unit}")], start=1):
        ...
        if row_idx % 2 == 0:
            for cell in row.cells:
                tc = cell._tc; tcPr = tc.get_or_add_tcPr(); shd = OxmlElement("w:shd")
                shd.set(qn("w:val"),"clear"); shd.set(qn("w:color"),"auto"); shd.set(qn("w:fill"),"e8f6f8"); tcPr.append(shd)
    doc.add_paragraph(); add_heading("2. Critical Path")
    path_names = " > ".join(f"{l} ({activities[l]['name']})" for l in crit_path)
    p = doc.add_paragraph(); run = p.add_run(path_names); run.bold = True; run.font.name = "Calibri"; run.font.size = Pt(11); run.font.color.rgb = RGBColor(0xC0,0x00,0x00)
    ...
    for row_idx, (lbl, info) in enumerate(activities.items(), start=1):
        ...
        for col_idx, val in enumerate(vals):
            ...
            if is_crit:
                run.bold = True; run.font.color.rgb = RGBColor(0xC0,0x00,0x00)
                tc = cell._tc; tcPr = tc.get_or_add_tcPr(); shd = OxmlElement("w:shd")
                shd.set(qn("w:val"),"clear"); shd.set(qn("w:color"),"auto"); shd.set(qn("w:fill"),"fff0f0"); tcPr.append(shd)
    doc.add_paragraph(); add_heading("4. Interpretation")
    for bullet in [f"Mean project duration: {mean_dur:.1f} {unit.lower()}.", f"50% probability of completing within {float(np.percentile(durations,50)):.1f} {unit.lower()}.", f"For {sl}% service level, plan for {sl_val:.1f} {unit.lower()}.", f"Std deviation of {std_dur:.1f} {unit.lower()} reflects schedule uncertainty."]:
...
with st.sidebar:
    ...
    st.markdown("---")
    page = st.radio("", ["◈  Dashboard", "📋  User Guide", "ℹ️  About PERT"], label_visibility="collapsed")
    st.markdown("---")
    ...
    st.markdown('<div style="margin-top:1.5rem;padding:1rem;background:#f0fafb;border-radius:10px;border:1px solid #b2dde3;font-size:0.78rem;color:#2a6a74;line-height:1.6;"><strong>💡 Tip</strong><br>Upload a CSV/Excel or enter activities manually, then click Run Simulation.</div>', unsafe_allow_html=True)
...
st.markdown('''<div class="hero">
  ...
  <div class="hero-badges">
    ...
    <span class="hero-badge">↓ Excel & Word Export</span>
  </div>
...
if "Guide" in page:
    ...
    with col2:
        ...
        st.markdown('<div class="info-box" style="margin-top:1rem;"><strong>Example row:</strong><br><code>B, Build Prototype, A, 3, 6, 9</code><br>Leave predecessors blank for first activity.</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="card"><div class="section-label">Critical Path Method</div>
          <p>The critical path is the longest sequence of dependent activities. It determines the minimum possible project duration. Delays on this path directly delay the project end date.</p>
    ...
    with left_col:
        tabs = st.tabs(["📋 Activities", "⬆️ Upload File"])
        with tabs[0]:
        ...
        with tabs[1]:
            st.markdown('<div class="info-box">Upload a <strong>CSV or Excel</strong> file with columns:<br><code>label, name, predecessors, min, avg, max</code></div>', unsafe_allow_html=True)
            ...
            st.download_button("⬇️ Download CSV Template", data=template_df.to_csv(index=False).encode(), file_name="pert_template.csv", mime="text/csv", use_container_width=True)
        ...
        if st.session_state.results:
            ...
            with rtabs[3]:
                st.markdown(f'''<div class="card card-accent">
                  <div class="section-label">Summary & Recommendations</div>
                  <p>Ran <strong>{n_sim:,} simulations</strong> in <strong>{unit.lower()}</strong>.</p>
                  <p>Expected duration: <strong style="color:{TEAL}">{mean_dur:.1f} {unit.lower()}</strong> (avg). For <strong style="color:{RED}">{sl}% on-time probability</strong>, plan for <strong style="color:{RED}">{sl_val:.1f} {unit.lower()}</strong>.</p>
                  <p>Critical path: <strong style="color:{RED}">{" > ".join(res["crit_path"])}</strong>. Focus risk management on these activities.</p>
                  <p>Non-critical activities have schedule float and their resources can be reallocated without affecting the end date.</p>
                </div>''', unsafe_allow_html=True)
            st.markdown("---")
            ...
            with exp_col1:
                try:
                    ...
                    st.download_button("⬇️ Download Excel Report", data=excel_data, file_name="projectpert_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            ...
            with exp_col2:
                try:
                    ...
                    st.download_button("⬇️ Download Word Report", data=word_data, file_name="projectpert_report.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
