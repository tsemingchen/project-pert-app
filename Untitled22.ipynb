 ...
 "cells": [
  {
   "cell_type": "code",
   "execution_count": 4,
   "id": "5c78cbb2-9dbd-4889-acb3-f39f5e364ca4",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
     "output_type": "stream",
     "text": [
      "✅ File saved! Now open a Terminal and run:\n",
      "   streamlit run /Users/minchen/Desktop/project_pert_app.py\n",
      "🚀 Launching app...\n",
      "\n",
      "  You can now view your Streamlit app in your browser.\n",
      "\n",
      "  Local URL: http://localhost:8504\n",
      "  Network URL: http://128.189.28.206:8504\n",
      "\n",
    ...
    },
    {
     ...
     "output_type": "stream",
     "text": [
      "2026-04-15 11:46:52.860 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 11:47:13.592 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 11:47:13.962 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n"
   ...
   "source": [
    ...
    ".hero-title { font-family: 'Space Mono', monospace; font-size: 2.6rem; font-weight: 700; color: #ffffff; margin: 0 0 0.4rem 0; }\n",
    ".hero-sub { color: rgba(255,255,255,0.75); font-size: 1.1rem; margin: 0; }\n",
    ".card { background: #ffffff; border: 1px solid #dde1f0; border-radius: 12px; padding: 1.5rem; margin-bottom: 1.2rem; box-shadow: 0 2px 8px rgba(99,102,241,0.07); }\n",
    ".section-label { font-family: 'Space Mono', monospace; font-size: 0.7rem; letter-spacing: 3px; text-transform: uppercase; color: #6366f1; margin-bottom: 1rem; }\n",
    ".metric-row { display: flex; gap: 1rem; margin-bottom: 1.5rem; }\n",
    ".metric-tile { flex: 1; background: #ffffff; border: 1px solid #dde1f0; border-radius: 10px; padding: 1.4rem; text-align: center; box-shadow: 0 2px 8px rgba(99,102,241,0.07); }\n",
    ".metric-val { font-family: 'Space Mono', monospace; font-size: 2.2rem; font-weight: 700; color: #4f46e5; }\n",
    ".metric-lbl { font-size: 0.85rem; color: #888; margin-top: 0.3rem; }\n",
    ".stButton>button { background: linear-gradient(135deg, #4f46e5, #7c3aed); color: white; border: none; border-radius: 8px; padding: 0.65rem 2rem; font-family: 'Space Mono', monospace; font-size: 0.9rem; }\n",
    "label { color: #444 !important; font-size: 0.9rem !important; }\n",
    ...
    "        critical_pred = max(preds, key=lambda p: earliest_finish[p])\n",
    "        return backtrack(critical_pred) + [node]\n",
    ...
    "\n",
    "st.markdown(\\'\\'\\'<div class=\"hero\"><div class=\"hero-title\">ProjectPERT</div>\n",
    "<p class=\"hero-sub\">Monte Carlo · Critical Path · Duration Forecaster</p></div>\\'\\'\\', unsafe_allow_html=True)\n",
    "\n",
    ...
    "    rows = [{\"Label\": l, \"Activity\": i[\"name\"], \"Predecessors\": \", \".join(i[\"predecessors\"]) or \"—\", \"Min\": i[\"min\"], \"Avg\": i[\"avg\"], \"Max\": i[\"max\"]} for l, i in acts.items()]\n",
    ...
    "        with c1: new_min = st.number_input(\"Min\", min_value=0, value=1)\n",
    "        with c2: new_avg = st.number_input(\"Avg\", min_value=0, value=3)\n",
    "        with c3: new_max = st.number_input(\"Max\", min_value=0, value=5)\n",
    ...
    "          <div class=\"metric-tile\"><div class=\"metric-val\">{mean_dur:.1f}</div><div class=\"metric-lbl\">Mean Duration (wks)</div></div>\n",
    "          <div class=\"metric-tile\"><div class=\"metric-val\">{std_dur:.1f}</div><div class=\"metric-lbl\">Std Deviation</div></div>\n",
    "          <div class=\"metric-tile\"><div class=\"metric-val\">{sl_val:.1f}</div><div class=\"metric-lbl\">{sl}% Service Level (wks)</div></div>\n",
    "          <div class=\"metric-tile\"><div class=\"metric-val\">{res[\"pert_dur\"]:.1f}</div><div class=\"metric-lbl\">PERT Expected (wks)</div></div>\n",
    ...
    "          <div style=\"font-size:1rem;color:#dc2626;line-height:1.9;\">{path_str}</div></div>\\'\\'\\', unsafe_allow_html=True)\n",
    "\n",
    ...
    "        ax.set_xlabel(\"Project Duration (weeks)\", color=\"#555\", fontsize=12)\n",
    "        ax.set_ylabel(\"Frequency\", color=\"#555\", fontsize=12)\n",
    "        ax.tick_params(colors=\"#555\", labelsize=11)\n",
    "        for spine in ax.spines.values(): spine.set_edgecolor(\"#dde1f0\")\n",
    "        ax.legend(facecolor=\"#ffffff\", edgecolor=\"#dde1f0\", labelcolor=\"#333\", fontsize=11)\n",
    "        fig.tight_layout(pad=1.5)\n",
    "        st.pyplot(fig)\n",
    "        plt.close(fig)\n",
    "\n",
    "        st.markdown(\\'<div class=\"section-label\">⬡ PERT Activity Breakdown</div>\\', unsafe_allow_html=True)\n",
    "        breakdown = [{\"Label\": lbl, \"Activity\": info[\"name\"], \"PERT Mean\": round(pert_mean(info[\"min\"], info[\"avg\"], info[\"max\"]), 2), \"Std Dev\": round(pert_std(info[\"min\"], info[\"max\"]), 2), \"Critical\": \"✅\" if lbl in res[\"crit_path\"] else \"\"} for lbl, info in acts_ref.items()]\n",
    ...
    "            <strong style=\"color:#4f46e5\">{mean_dur:.1f} weeks</strong> on average.\n",
    "            For a <strong style=\"color:#dc2626\">{sl}% service level</strong>, plan for\n",
    "            <strong style=\"color:#dc2626\">{sl_val:.1f} weeks</strong>.\n",
    ...
    "          <div style=\"font-size:1rem;color:#999;\">Configure activities on the left<br>then click\n",
    "            <span style=\"color:#6366f1\">▶ Run Simulation</span></div>\n",
    "        </div>\\'\\'\\', unsafe_allow_html=True)\n",
    "'''\n",
    "\n",
    "# Write the file\n",
    ...
    "    f.write(code)\n",
    "\n",
    "print(\"✅ File saved! Now open a Terminal and run:\")\n",
    "print(\"   streamlit run /Users/minchen/Desktop/project_pert_app.py\")\n",
    "\n",
    "# Optional: auto-launch it\n",
    "import subprocess\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": 5,
   "id": "3f56bb01-bb8b-454e-875e-aaf4e787e950",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
     ...
     "text": [
      "✅ File saved!\n",
      "🚀 App launching in your browser...\n",
      "\n",
      "  You can now view your Streamlit app in your browser.\n",
      "\n",
      "  Local URL: http://localhost:8505\n",
      "  Network URL: http://128.189.28.206:8505\n",
      "\n",
      "  For better performance, install the Watchdog module:\n",
      "\n",
      "  $ xcode-select --install\n",
    ...
    },
    {
     ...
     "output_type": "stream",
     "text": [
      "2026-04-15 13:49:50.819 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 13:49:57.544 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 13:49:58.022 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 13:49:58.025 Uncaught app execution\n",
      "Traceback (most recent call last):\n",
      "  File \"/Users/minchen/miniforge3/lib/python3.12/site-packages/streamlit/runtime/scriptrunner/exec_code.py\", line 129, in exec_func_with_error_handling\n",
      "    result = func()\n",
      "             ^^^^^^\n",
      "  File \"/Users/minchen/miniforge3/lib/python3.12/site-packages/streamlit/runtime/scriptrunner/script_runner.py\", line 689, in code_to_exec\n",
      "    exec(code, module.__dict__)  # noqa: S102\n",
      "    ^^^^^^^^^^^^^^^^^^^^^^^^^^^\n",
      "  File \"/Users/minchen/Desktop/project_pert_app.py\", line 355, in <module>\n",
      "    excel_data = export_to_excel(acts_ref, durations, res[\"pert_dur\"], res[\"crit_path\"], sl, unit)\n",
      "                 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n",
      "  File \"/Users/minchen/Desktop/project_pert_app.py\", line 94, in export_to_excel\n",
      "    with pd.ExcelWriter(output, engine=\"xlsxwriter\") as writer:\n",
      "         ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n",
      "  File \"/Users/minchen/miniforge3/lib/python3.12/site-packages/pandas/io/excel/_xlsxwriter.py\", line 198, in __init__\n",
   ...
   "source": [
    ...
    "<style>\n",
    "@import url('https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&family=DM+Sans:wght@300;400;500;600&display=swap');\n",
    "html, body, [class*=\"css\"] { font-family: \\'DM Sans\\', sans-serif; font-size: 20px; }\n",
    ".stApp { background-color: #f0f2f8; color: #1a1a2e; }\n",
    "#MainMenu, footer, header { visibility: hidden; }\n",
    ".hero { background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%); border-radius: 16px; padding: 2.5rem 3rem; margin-bottom: 2rem; }\n",
    ".hero-title { font-family: \\'Space Mono\\', monospace; font-size: 3rem; font-weight: 700; color: #ffffff; margin: 0 0 0.4rem 0; }\n",
    ".hero-sub { color: rgba(255,255,255,0.75); font-size: 1.3rem; margin: 0; }\n",
    ".card { background: #ffffff; border: 1px solid #dde1f0; border-radius: 12px; padding: 1.5rem; margin-bottom: 1.2rem; box-shadow: 0 2px 8px rgba(99,102,241,0.07); }\n",
    ".section-label { font-family: \\'Space Mono\\', monospace; font-size: 0.85rem; letter-spacing: 3px; text-transform: uppercase; color: #6366f1; margin-bottom: 1rem; }\n",
    ".metric-row { display: flex; gap: 1rem; margin-bottom: 1.5rem; }\n",
    ".metric-tile { flex: 1; background: #ffffff; border: 1px solid #dde1f0; border-radius: 10px; padding: 1.4rem; text-align: center; box-shadow: 0 2px 8px rgba(99,102,241,0.07); }\n",
    ".metric-val { font-family: \\'Space Mono\\', monospace; font-size: 2.6rem; font-weight: 700; color: #4f46e5; }\n",
    ".metric-lbl { font-size: 1rem; color: #888; margin-top: 0.3rem; }\n",
    ".stButton>button { background: linear-gradient(135deg, #4f46e5, #7c3aed); color: white; border: none; border-radius: 8px; padding: 0.75rem 2rem; font-family: \\'Space Mono\\', monospace; font-size: 1rem; }\n",
    ".stButton>button:hover { opacity: 0.88; }\n",
    "label, .stTextInput label, .stNumberInput label, .stSlider label, .stSelectbox label, .stMultiSelect label { color: #333 !important; font-size: 1rem !important; font-weight: 500 !important; }\n",
    "div[data-testid=\"stNumberInput\"] input, div[data-testid=\"stTextInput\"] input { font-size: 1rem !important; }\n",
    "hr { border-color: #dde1f0; }\n",
    ...
    "\n",
    "        # ── Styles ──\n",
    "        title_fmt = workbook.add_format({\"bold\": True, \"font_size\": 16, \"font_color\": \"#4f46e5\", \"bottom\": 2, \"bottom_color\": \"#4f46e5\"})\n",
    "        header_fmt = workbook.add_format({\"bold\": True, \"bg_color\": \"#4f46e5\", \"font_color\": \"#ffffff\", \"font_size\": 12, \"border\": 1, \"border_color\": \"#dde1f0\", \"align\": \"center\"})\n",
    "        cell_fmt = workbook.add_format({\"font_size\": 11, \"border\": 1, \"border_color\": \"#dde1f0\"})\n",
    "        num_fmt = workbook.add_format({\"font_size\": 11, \"border\": 1, \"border_color\": \"#dde1f0\", \"num_format\": \"0.00\", \"align\": \"center\"})\n",
    "        highlight_fmt = workbook.add_format({\"font_size\": 11, \"border\": 1, \"border_color\": \"#dde1f0\", \"bg_color\": \"#fef2f2\", \"font_color\": \"#dc2626\", \"bold\": True})\n",
    "        metric_label_fmt = workbook.add_format({\"bold\": True, \"font_size\": 12, \"bg_color\": \"#eef2ff\", \"border\": 1, \"border_color\": \"#dde1f0\"})\n",
    "        metric_val_fmt = workbook.add_format({\"font_size\": 12, \"border\": 1, \"border_color\": \"#dde1f0\", \"num_format\": \"0.00\", \"font_color\": \"#4f46e5\", \"bold\": True})\n",
    "\n",
    "        # ════════════════════════════════\n",
    "        # Sheet 1: Summary\n",
    "        # ════════════════════════════════\n",
    ...
    "\n",
    "        ws.write(\"A1\", \"ProjectPERT — Simulation Summary\", title_fmt)\n",
    "        ws.write(\"A2\", f\"Time unit: {unit}\")\n",
    "\n",
    "        metrics = [\n",
    ...
    "            row = i + 3\n",
    "            ws.write(row, 0, label, metric_label_fmt)\n",
    "            ws.write(row, 1, val, metric_val_fmt)\n",
    "\n",
    "        ws.write(8, 0, \"Critical Path\", metric_label_fmt)\n",
    "        ws.write(8, 1, \" → \".join(crit_path), workbook.add_format({\"font_size\": 11, \"border\": 1, \"font_color\": \"#dc2626\", \"bold\": True}))\n",
    "\n",
    "        # ════════════════════════════════\n",
    "        # Sheet 2: Activities\n",
    "        # ════════════════════════════════\n",
    "        ws2 = workbook.add_worksheet(\"Activities\")\n",
    "        writer.sheets[\"Activities\"] = ws2\n",
    "        ws2.set_column(\"A:A\", 8)\n",
    "        ws2.set_column(\"B:B\", 28)\n",
    "        ws2.set_column(\"C:C\", 22)\n",
    "        ws2.set_column(\"D:F\", 14)\n",
    "        ws2.set_column(\"G:H\", 14)\n",
    "        ws2.set_column(\"I:I\", 10)\n",
    "\n",
    "        headers = [\"Label\", \"Activity\", \"Predecessors\", f\"Min ({unit})\", f\"Avg ({unit})\", f\"Max ({unit})\", \"PERT Mean\", \"Std Dev\", \"Critical\"]\n",
    ...
    "            is_crit = lbl in crit_path\n",
    "            fmt = highlight_fmt if is_crit else cell_fmt\n",
    ...
    "            ws2.write(row, 8, \"YES\" if is_crit else \"\", fmt)\n",
    "\n",
    "        # ════════════════════════════════\n",
    "        # Sheet 3: Simulation Data\n",
    "        # ════════════════════════════════\n",
    "        ws3 = workbook.add_worksheet(\"Simulation Data\")\n",
    "        writer.sheets[\"Simulation Data\"] = ws3\n",
    "        ws3.set_column(\"A:B\", 20)\n",
    "        ws3.write(0, 0, \"Simulation #\", header_fmt)\n",
    "        ws3.write(0, 1, f\"Duration ({unit})\", header_fmt)\n",
    "        sample = durations[:5000]  # cap at 5k rows for Excel\n",
    ...
    "        # ════════════════════════════════\n",
    "        ws4 = workbook.add_worksheet(\"Histogram\")\n",
    "        writer.sheets[\"Histogram\"] = ws4\n",
    "        ws4.set_column(\"A:B\", 22)\n",
    "        ws4.write(0, 0, f\"Duration Bin ({unit})\", header_fmt)\n",
    "        ws4.write(0, 1, \"Frequency\", header_fmt)\n",
    ...
    "        path_str = \" → \".join(f\"{l} ({acts_ref[l][\\'name\\']})\" for l in res[\"crit_path\"])\n",
    "        st.markdown(f\\'\\'\\'<div class=\"card\"><div class=\"section-label\">⚡ Critical Path</div>\n",
    "          <div style=\"font-size:1.1rem;color:#dc2626;line-height:2;\">{path_str}</div></div>\\'\\'\\', unsafe_allow_html=True)\n",
    "\n",
    "        st.markdown(\\'<div class=\"section-label\">◉ Duration Distribution</div>\\', unsafe_allow_html=True)\n",
    "        fig, ax = plt.subplots(figsize=(9, 4))\n",
    "        fig.patch.set_facecolor(\"#ffffff\")\n",
    ...
    "        ax.axvline(mean_dur, color=\"#4f46e5\", linewidth=2, linestyle=\"--\", label=f\"Mean: {mean_dur:.1f}\")\n",
    "        ax.axvline(sl_val,   color=\"#dc2626\", linewidth=2, linestyle=\":\",  label=f\"{sl}th pct: {sl_val:.1f}\")\n",
    "        ax.set_xlabel(f\"Project Duration ({unit})\", color=\"#555\", fontsize=13)\n",
    "        ax.set_ylabel(\"Frequency\", color=\"#555\", fontsize=13)\n",
    "        ax.tick_params(colors=\"#555\", labelsize=12)\n",
    "        for spine in ax.spines.values(): spine.set_edgecolor(\"#dde1f0\")\n",
    "        ax.legend(facecolor=\"#ffffff\", edgecolor=\"#dde1f0\", labelcolor=\"#333\", fontsize=12)\n",
    "        fig.tight_layout(pad=1.5)\n",
    "        st.pyplot(fig)\n",
    "        plt.close(fig)\n",
    "\n",
    ...
    "                      \"Critical\": \"✅\" if lbl in res[\"crit_path\"] else \"\"}\n",
    "                     for lbl, info in acts_ref.items()]\n",
    "        st.dataframe(pd.DataFrame(breakdown), use_container_width=True, hide_index=True)\n",
    "\n",
    "        st.markdown(f\\'\\'\\'<div class=\"card\" style=\"margin-top:1rem;\">\n",
    "          <div class=\"section-label\">📝 Interpretation</div>\n",
    "          <p style=\"color:#333;font-size:1.05rem;line-height:1.9;margin:0\">\n",
    "            Based on <strong style=\"color:#4f46e5\">{n_sim:,}</strong> simulations, the project is expected to take\n",
    ...
    "            border:2px dashed #c7cce8;border-radius:14px;background:#ffffff;text-align:center;gap:1rem;\">\n",
    "          <div style=\"font-size:3.5rem;\">📊</div>\n",
    "          <div style=\"font-size:1.1rem;color:#999;\">Configure activities on the left<br>then click\n",
    "            <span style=\"color:#6366f1\">▶ Run Simulation</span></div>\n",
    "        </div>\\'\\'\\', unsafe_allow_html=True)\n",
    "'''\n",
    "\n",
    ...
    "import subprocess\n",
    "subprocess.Popen([\"streamlit\", \"run\", \"/Users/minchen/Desktop/project_pert_app.py\"])\n",
    "print(\"🚀 App launching in your browser...\")"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": 8,
   "id": "c88d27ea-9647-4f32-9c0f-060db559b7e6",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
     ...
     "text": [
      ...
      "  Network URL: http://128.189.28.206:8507\n",
      "\n",
      "  For better performance, install the Watchdog module:\n",
      "\n",
      "  $ xcode-select --install\n",
    ...
    },
    {
     ...
     "output_type": "stream",
     "text": [
      "2026-04-15 14:32:11.499 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:14.620 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:14.663 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:16.979 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:17.038 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:19.601 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:20.552 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:25.102 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n",
      "2026-04-15 14:32:25.539 Please replace `use_container_width` with `width`.\n",
      "\n",
      "`use_container_width` will be removed after 2025-12-31.\n",
      "\n",
      "For `use_container_width=True`, use `width='stretch'`. For `use_container_width=False`, use `width='content'`.\n"
  ...
  },
  {
   "cell_type": "code",
   "execution_count": 7,
   "id": "680047e6-3881-40f4-88e1-624ed7400c51",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
   ...
   "source": [
    "import subprocess\n",
    "subprocess.run([\"/Users/minchen/miniforge3/bin/pip\", \"install\", \"xlsxwriter\"], check=True)\n",
    "print(\"✅ Done!\")"
   ]
  },
  {
