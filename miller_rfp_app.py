...
with st.sidebar:
    ...
    comp1_mode = st.number_input("Most likely bid ($)", value=130_000, step=1_000, key="c1mode")
    comp1_max  = st.number_input("Max bid ($)",         value=180_000, step=1_000, key="c1max")

    st.markdown("**Two uncertain competitors (each 50% chance)**")
    comp2_prob = st.slider("Probability each enters (%)", 0, 100, 50, key="c2prob") / 100
    comp2_min  = st.number_input("Min bid ($)",         value=90_000,  step=1_000, key="c2min")
    comp2_mode = st.number_input("Most likely bid ($)", value=130_000, step=1_000, key="c2mode")
